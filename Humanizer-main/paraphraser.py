...
def load_model(model_name_param: str = None) -> Tuple[bool, Optional[str]]:
    ...
    try:
        ...
        if config["requires_sentencepiece"]:
            ...
            model = T5ForConditionalGeneration.from_pretrained(config["model_name"], cache_dir="models")
        else:
            # BART/Pegasus models
            from transformers import AutoTokenizer, AutoModelForSeq2SeqLM, pipeline
            tokenizer = AutoTokenizer.from_pretrained(config["model_name"], cache_dir="models")
            model = AutoModelForSeq2SeqLM.from_pretrained(config["model_name"], cache_dir="models")
